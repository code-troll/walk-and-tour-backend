...
- Both provider abstractions default to their `console` implementation, which is correct for development and a silent failure in production: the application boots, requests succeed, the log records what would have happened, and nothing is delivered. Production ran for a while with `EMAIL_PROVIDER=console` for exactly that reason. The bootstrap now warns when a `console` provider is selected in production, when Resend is selected without an API key, and when the Auth0 identity provider is selected without its management credentials or issuer. These are warnings rather than a refusal to start, because a half-configured deployment is better off running than not.
- Auth0's New Universal Login does not return the hotel anywhere after a password ticket is used: `result_url` is ignored, passing `client_id` instead does not help, and passing both makes Auth0 refuse the ticket outright. `result_url` is still sent because it is the correct request and costs nothing, and the invitation email now carries the portal address so the hotel has a way back regardless.
